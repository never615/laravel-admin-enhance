...
trait AdminSubjectTrait
{
    ...
    protected function gridSubject($grid)
    {
        if (Schema::hasColumn($this->tableName, "subject_id")) {
            ...
            if (AdminUtils::isOwner() || Admin::user()->subject->hasChildrenSubject()) {
                $grid->subject_id(trans('admin.subjects'))->sortable()->display(function ($value) {
    ...
    protected function formSubject($form)
    {
        if (Schema::hasColumn($this->tableName, "subject_id")) {
            ...
            if (\Mallto\Admin\AdminUtils::isOwner()) {
                $form->selectE("subject_id", trans('admin.subjects'))
                    ->options(
    ...
    protected function autoSubjectSaving($form)
    {
        ...
        if (Schema::hasColumn($this->tableName, "subject_id") &&
            ...
            //有提交 subject_id 则使用提交的
            if ($form->subject_id && $form->subject_id != $form->model()->subject_id) {

            } else {
                if ( ! $form->model()->subject_id) {
