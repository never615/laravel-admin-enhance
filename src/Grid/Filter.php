...
class Filter
{
    ...
    public function conditionsByInputs($inputs)
    {
        ...
        $conditions = [];
        foreach ($this->filters() as $filter) {
            $conditions[] = $filter->condition($params, true);
        }
    ...
    public function executeForQuery($inputs = null, $subject_id, $whereExistCallback)
    {
        if (is_null($inputs)) {
            $inputs = Input::all();
        }

        return $this->model->addConditions($this->conditionsByInputs($inputs))->buildQuery($subject_id, true,
            $whereExistCallback);
    }
...
EOT;
    ...
    protected function urlWithoutFilters()
    {
        ...
        $url = count($request->query()) > 0
            ? $request->url().$question.http_build_query($query)
            : $request->fullUrl();

        $url = \Mallto\Tool\Utils\AppUtils::checkHttpProtocol($url);
