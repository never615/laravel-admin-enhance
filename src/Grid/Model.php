...
class Model
{
     ...
     * @param      $whereExistCallback
     * @return EloquentModel
     */
    public function buildQuery($subject_id, $isDbQuery = false, $whereExistCallback)
    {
        return $this->getQuery($subject_id, $isDbQuery, $whereExistCallback);
    }
     ...
     * @return EloquentModel
     */
    protected function getQuery($subject_id, $isDbQuery = false, $whereExistCallback)
    {
        if ($isDbQuery) {
        ...
        $this->queries->unique()->each(function ($query) use (&$dbQuery, $subject_id, $isDbQuery, $whereExistCallback) {
            if (($query['method'] == "dynamicData" || $query['method'] == "with")) {
                if (!$isDbQuery) {
                    $dbQuery = call_user_func_array([$dbQuery, $query['method']], $query['arguments']);
                }
            } else {
                if ($query['method'] == "whereExists") {
                    $dbQuery = call_user_func_array($whereExistCallback, [$dbQuery, $query['arguments']]);
                } else {
                    $dbQuery = call_user_func_array([$dbQuery, $query['method']], $query['arguments']);
                }
            }
